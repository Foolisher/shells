...
软件设计中分而治之一直是软件设计与规划中比较重要的一部分内容，而切分的一个有效手段就是模块化，利用模块化将系统进行拆解，让系统是灵活的，可扩展的，结构清晰的。在我们以往使用Spring的经验中，用一个xml文件来定义一个业务模块包含的bean是常见的用法，在在其他文件中用 **import** 将其组织起来，这就是模块化使用的一种表现，而 **Guice** 提供了一种编码方式的模块化组织方式，
