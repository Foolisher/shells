...
具体逻辑是抽象化了的，同时对使用者又是透明的，检验抽象程度的合理性，检验符合接口原则。如果抽象了，还发现里面有大把的`instanceof`断言来路由不同的实例，那么说明抽象程度是不够或不对的，若抽象了还要对不同实例不同处理还干嘛抽象呢？因此在抽象的入口逻辑，应当对实现一视同仁，否则就是违背了LSR。
